...
一个页面只保留支撑阅读路径的强调；Callout 这类会打断主线的重点块每章最多一个；Toggle、Table、Empty block、Media、Database 等常规块按需使用，一章可以有多个。不为丰富而堆块。
...
多个内容互相切换、并列查看时用 Tabs（适合方案对比、多版本说明），不适合承载长文。

标题是 tab 内的第一行缩进文本，不是属性；图标用 icon 属性（emoji 或 Notion Icon）：
...
图标和颜色是建议不是限制：场景明确匹配时用表中的组合，特殊情境可以自选更贴切的 icon 或颜色。
...
- 一组同类卡片块（多个 Callout、mention-page、Toggle 组成的组）整体前后各一个；组内不插空行
- 列表/表格整体前后各一个
